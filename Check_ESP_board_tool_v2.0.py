#!/usr/bin/env python3
# ultimate_esp_inspector_v2.0.py
# A comprehensive ESP Hardware Inspector Suite.
# v2.0: The "Automation" Edition.
# - Implemented automatic baud rate detection for the Hardware Inspector.
# - Reworked UI layout to be fully responsive to window resizing.
# - Final code review for integrity and robustness.
...
try:
    ...
    import serial # Import the full serial library for the monitor
    import esptool
except ImportError:
    print("FATAL ERROR: Required packages 'pyserial' or 'esptool' not found.")
    sys.exit(1)

# ADDED "Tự động" to the list
...
# ... (Các hàm Core Logic và Parsing giữ nguyên) ...
...
class InspectorApp:
    def __init__(self, root):
        self.root = root; self.root.title("ESP Hardware Inspector v2.0")
    ...
    def create_widgets(self):
        ...
        main_frame.columnconfigure(0, weight=1) # Allow notebook to expand
        ...
        self.notebook.grid(row=1, column=0, columnspan=3, sticky="nsew", pady=5)
    ...
    def on_tab_change(self, event):
        is_monitor_mode = self.serial_connection is not None and self.serial_connection.is_open
        if is_monitor_mode:
            self.notebook.select(self.monitor_tab)
            messagebox.showinfo("Đang kết nối", "Vui lòng ngắt kết nối khỏi Serial Monitor trước khi chuyển sang tab khác.")
    ...
    def on_serial_connect(self):
        if self.baud_cb.get() == "Tự động":
            messagebox.showinfo("Thông báo", "Tính năng 'Tự động' Baudrate chỉ khả dụng cho tab 'Kiểm tra Phần cứng'.\nVui lòng chọn một tốc độ Baud cụ thể cho Serial Monitor.")
            return
        ...
        try:
            ...
            self.notebook.tab(self.inspector_tab, state="disabled")
    ...
    def on_serial_disconnect(self):
        ...
        self.set_status("Đã ngắt kết nối."); self.notebook.tab(self.inspector_tab, state="normal")
        ...
        while not self.stop_serial_thread.is_set():
            if self.serial_connection and self.serial_connection.is_open:
                try:
                    ...
                    if data: self.root.after(0, self.append_to_monitor, data.decode('utf-8', errors='replace'))
    ...
    def append_to_monitor(self, text):
        self.monitor_txt.config(state="normal"); self.monitor_txt.insert("end", text)
